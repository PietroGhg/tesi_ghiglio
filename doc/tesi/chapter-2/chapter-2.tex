...
Performance counters are a feature of some hardware architectures, that expose some registers that store information about the energy consumption of a program. In the following section we will give an overview of Intel's RAPL as an example of such a feature.
\par RAPL provides a set of counters providing energy and power information. It is not an analog power meter, but rather uses a software power model that estimates power consumption by means of performance counters and hardware power models \cite{rapl-link}. Basing on \cite{rapl}, this predictions matches actual measurements. The RAPL counters can be accessed by a user by reading appropriate files on the target machine. A typical usage of RAPL is to read the energy measures, perform some actions, and then read again the energy the energy measures, taking the difference between the two readings as the estimate of the consumed energy. 
...
use direct measure as ground truth, it shows that both techniques have very high error.\\[1in]

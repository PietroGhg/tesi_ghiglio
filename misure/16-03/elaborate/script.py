...
res['symm 10 100'] = float(res['symm 10 100']) / 4.0
k = list(res.keys())
v = list(res.values())
x = [str(i) for i in range(len(k))]

p1 = pyplot.subplot(2,1,1)
#pyplot.plot(x,v,'bo',label='measure')
...
est_fixed['symm 10 100'] = float(est_fixed['symm 10 100']) / 4.0
k2 = list(est_fixed.keys())
v2 = list(est_fixed.values())

#pyplot.plot(x,v2,'go',label='estimate')
pyplot.bar(x,v2,color='red',label='estimates')
p1.legend()
